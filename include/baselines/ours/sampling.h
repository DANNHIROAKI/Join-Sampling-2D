...
class RangePointSegTree {
 public:
  void Init(u32 num_handles, u32 num_ranks) {
    ...
    // This is the depth of a leaf node in the implicit tree.
    leaf_depth_ = 0;
    for (u32 x = p_; x > 1; x >>= 1) ++leaf_depth_;

    // Path length leaf->root inclusive.
    // We use backref = 0..leaf_depth_ where backref increments as we walk
    // leaf->root. This is faster than recomputing backref from node depth.
  ...
  void ResetActive() {
    // Fast path: if nothing is active, the structure is already empty.
    // This matters because some codepaths call ResetActive() right after Init()
    // and/or at the beginning of a sweep.
  ...
  // This avoids repeated growth/reallocation on the root/top-level buckets during the sweep.
  void ReserveHotBuckets(u32 max_active, u32 levels = 4) {
    if (max_active == 0 || nodes_.empty()) return;
    for (u32 depth = 0; depth < levels; ++depth) {
      const u32 first = 1U << depth;
      const u32 last = 1U << (depth + 1U);
      const u32 nodes_at = last - first;
      const u32 cap_per = std::max<u32>(1U, max_active / nodes_at);
      for (u32 node = first; node < last && node < nodes_.size(); ++node) {
        if (nodes_[node].items.capacity() < cap_per) {
          nodes_[node].items.reserve(cap_per);
        }
      }
    }
  }
  ...
  void Insert(u32 handle, u32 rank) {
    ...
    u32 backref = 0;
    while (idx > 0) {
      AddToNode(handle, idx, base, backref);
      idx >>= 1;
      ++backref;
    }
  ...
  void Erase(u32 handle) {
    ...
    u32 backref = 0;
    while (idx > 0) {
      const u32 pos = pos_in_node_[base + static_cast<usize>(backref)];
      RemoveFromNode(idx, pos, backref);
      idx >>= 1;
      ++backref;
    }
    SJS_DASSERT(backref == max_refs_);
 ...
 private:
  ...
  void AddToNode(u32 handle, u32 node, usize base, u32 backref) {
    SJS_DASSERT(node < nodes_.size());

    SJS_DASSERT(backref < max_refs_);
    SJS_DASSERT(base + static_cast<usize>(backref) < pos_in_node_.size());

    auto& bucket = nodes_[node].items;
    const u32 pos = static_cast<u32>(bucket.size());

    pos_in_node_[base + static_cast<usize>(backref)] = pos;
    bucket.push_back(handle);
  }

  void RemoveFromNode(u32 node, u32 pos, u32 backref) {
    SJS_DASSERT(node < nodes_.size());
    auto& bucket = nodes_[node].items;
    SJS_DASSERT(!bucket.empty());
    ...
    if (static_cast<usize>(pos) != last_pos) {
      ...
      pos_in_node_[static_cast<usize>(swapped_handle) * static_cast<usize>(max_refs_) + static_cast<usize>(backref)] = pos;
    }
    bucket.pop_back();
  }
...
class StabbingSegTree {
 public:
  // To reduce memory traffic in the sweep hot path, each bucket item is packed into a single u32:
  //   packed = (handle << backref_shift_) | backref
  // backref_shift_ is computed in Init() as ceil_log2(max_refs_).
  ...
  void Init(u32 num_handles, u32 num_points) {
    ...
    max_refs_ = std::max<u32>(1U, 2U * logp);

    // Compute packing parameters for (handle, backref) -> u32.
    // We keep backref in the low bits; number of bits is ceil_log2(max_refs_).
    {
      u32 pow2 = 1;
      u32 shift = 0;
      while (pow2 < max_refs_) {
        pow2 <<= 1;
        ++shift;
      }
      backref_shift_ = shift;
      backref_mask_ = pow2 - 1U;
      // Ensure handle fits in the remaining high bits.
      if (backref_shift_ > 0) {
        const u64 max_handles = (1ULL << (32U - backref_shift_));
        SJS_DASSERT(static_cast<u64>(n_handles_) <= max_handles);
      }
    }
  ...
  // Optional performance hint: reserve bucket capacities for hot nodes.
  // Similar heuristic to RangePointSegTree::ReserveHotBuckets().
  void ReserveHotBuckets(u32 max_active, u32 levels = 4) {
    if (max_active == 0 || nodes_.empty()) return;
    if (p_ == 0) return;

    // Depth 0 is root at node=1; leaves start at node=p_.
    // Reserve only internal nodes on the top levels.
    // We estimate per-node capacity as max_active / nodes_at_level.
    for (u32 depth = 0; depth < levels; ++depth) {
      const u32 first = 1U << depth;
      const u32 last = 1U << (depth + 1U);
      if (first >= p_) break;
      ...
      const u32 nodes_at = last - first;
      const u32 cap_per = std::max<u32>(1U, max_active / nodes_at);
      for (u32 node = first; node < last && node < p_; ++node) {
        if (nodes_[node].items.capacity() < cap_per) {
          nodes_[node].items.reserve(cap_per);
        }
      }
  ...
  void Insert(u32 handle, u32 L, u32 R) {
    ...
    while (l < r) {
      if (l & 1U) {
        AddToNode(handle, l, base, backref);
        ++backref;
        ++l;
      }
      if (r & 1U) {
        --r;
        AddToNode(handle, r, base, backref);
        ++backref;
      }
      l >>= 1;
      r >>= 1;
  ...
  bool Sample(u32 q, u32 k, Rng* rng, std::vector<u32>* out) const {
    ...
    if (n > 1 && k >= kAliasMinK) {
      ...
      for (u32 i = 0; i < k; ++i) {
        ...
        SJS_DASSERT(!bucket.empty());
        const u32 pos = rng->UniformU32(static_cast<u32>(bucket.size()));
        out->push_back(HandleOf(bucket[pos]));
      }
      return true;
    }
  ...
  void Report(u32 q, std::vector<u32>* out) const {
    ...
    while (idx > 0) {
      ...
      for (const u32 packed : bucket) out->push_back(HandleOf(packed));
      idx >>= 1;
    }
  }
 ...
 private:
  struct Node {
    std::vector<u32> items;  // packed (handle, backref)
  };
  ...
  u32 Pack(u32 handle, u32 backref) const noexcept {
    return (handle << backref_shift_) | backref;
  }

  u32 HandleOf(u32 packed) const noexcept { return (packed >> backref_shift_); }
  u32 BackrefOf(u32 packed) const noexcept { return (packed & backref_mask_); }
  ...
  void AddToNode(u32 handle, u32 node, usize base, u32 backref) {
    ...
    pos_in_node_[base + static_cast<usize>(backref)] = pos;
    bucket.push_back(Pack(handle, backref));
  }
  ...
  void RemoveFromNode(u32 node, u32 pos) {
    ...
    const usize last_pos = bucket.size() - 1;
    if (static_cast<usize>(pos) != last_pos) {
      const u32 swapped = bucket[last_pos];
      bucket[pos] = swapped;
      const u32 sh = HandleOf(swapped);
      const u32 sb = BackrefOf(swapped);
      pos_in_node_[static_cast<usize>(sh) * static_cast<usize>(max_refs_) + static_cast<usize>(sb)] = pos;
    }
    bucket.pop_back();
  }
...
class Ours2DContext {
 public:
  ...
  bool Build(const DatasetT& ds, PhaseRecorder* phases, std::string* err) {
    ...
    // Cache per-event endpoints (for both START and END events).
    // This makes the phase1/phase3 sweeps more cache-friendly by avoiding random
    // accesses into the per-relation ylo/yhi rank arrays.
    ev_ylo_rank_.resize(events_.size());
    ev_yhi_lb_rank_.resize(events_.size());
    for (usize pos = 0; pos < events_.size(); ++pos) {
      const auto& ev = events_[pos];
      if (ev.side == join::Side::R) {
        ev_ylo_rank_[pos] = ylo_rank_r_[ev.index];
        ev_yhi_lb_rank_[pos] = yhi_lb_rank_r_[ev.index];
      } else {
        ev_ylo_rank_[pos] = ylo_rank_s_[ev.index];
        ev_yhi_lb_rank_[pos] = yhi_lb_rank_s_[ev.index];
      }
    }
 ...
 private:
  ...
  // Per-event cached endpoints (for both START and END events).
  // This avoids random accesses into ylo/yhi arrays during the sweeps.
  std::vector<u32> ev_ylo_rank_;
  std::vector<u32> ev_yhi_lb_rank_;
...
inline bool BuildSlotPlan2D(u32 t,
  ...
  // Build an alias table over the 2E categories (event,pattern).
// Category c encodes (eid=c>>1, pat=c&1) with weight:
//   pat=0 -> w_a[eid], pat=1 -> w_b[eid].
std::vector<u64> w_pat;
w_pat.resize(2 * E);
for (usize e = 0; e < E; ++e) {
  w_pat[2 * e] = w_a[e];
  w_pat[2 * e + 1] = w_b[e];
...
if (!alias.BuildFromU64(Span<const u64>(w_pat), err)) {
  ...
  for (u32 j = 0; j < t; ++j) {
    const u32 info = static_cast<u32>(alias.Sample(rng));  // in [0, 2E)
